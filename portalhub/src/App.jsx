...
const goToVincereAuth = () => { window.location.href = '/api/vincere/auth'; };
...
function parseJob(j) {
  return { id:j.hashId||j.refnr||Math.random().toString(36),
    title:j.titel||'—', company:j.arbeitgeber||'—',
    city:j.arbeitsort?.ort||j.arbeitsort?.region||'—',
    region:j.arbeitsort?.region||'—',
    posted:formatAge(j.aktuelleVeroeffentlichungsdatum), type:mapA(j.angebotsart) };
}
function downloadCSV(jobs) {
  const h=['Titel','Unternehmen','Stadt','Veröffentlicht','Typ'];
  const rows=jobs.map(j=>[j.title,j.company,j.city,j.posted,j.type]);
...
const ANGEBOTE=[{v:'1',l:'Arbeitsstellen'},{v:'2',l:'Ausbildung'},{v:'4',l:'Praktikum'}];
...
function VBadge({company,vincereNames,onAdd,adding}) {
...
function JobCard({job,vincereNames,onAddVincere,addingId}) {
  ...
  const inV=vincereNames.some(n=>nameMatch(n,job.company));
  return <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
    ...
    <div style={{flex:1,minWidth:0}}>
      ...
      <div style={{fontSize:12,color:C.muted,display:'flex',gap:5}}><span>{job.company}</span><span style={{color:C.faint}}>·</span><span>{job.city}</span></div>
    </div>
    <div style={{display:'flex',alignItems:'center',gap:7,flexShrink:0}}>
      ...
      <VBadge company={job.company} vincereNames={vincereNames} onAdd={()=>onAddVincere(job.company)} adding={addingId===job.company} />
    </div>
  </div>;
}
...
function SearchView({vincereNames,onAddVincere,addingId,setSH,vincereConnected}) {
  ...
  const doSearch=useCallback(async(searchTerms,searchPage=1,append=false)=>{
    ...
    try {
      ...
      setTotal(data.maxErgebnisse||0); setPage(searchPage); setHasSearched(true);
  ...
  const addAllNew=async()=>{
    const news=[...new Set(jobs.map(j=>j.company).filter(c=>c&&c!=='—'&&!vincereNames.some(n=>nameMatch(n,c))))];
    if(!news.length)return;
    setBulkAdding(true);setBulkDone(0);
    for(const name of news){await onAddVincere(name);setBulkDone(d=>d+1);}
    setBulkAdding(false);
  };
  ...
  return <div>
    <div style={{marginBottom:20}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginTop:4}}>
        ...
        {vincereConnected&&<span style={{fontSize:10,fontWeight:700,background:C.violetDim,color:C.violetLight,border:'1px solid '+C.violetBorder,borderRadius:100,padding:'2px 8px'}}>Vincere verbunden</span>}
      </div>
    </div>
    ...
    <Card style={{padding:16,marginBottom:12}}>
      ...
      {terms.length>0&&<div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center',marginBottom:showFilters?14:0}}>
        {terms.map(t=><Tag key={t} label={t} onRemove={()=>setTerms(ts=>ts.filter(x=>x!==t))} />)}
      </div>}
      {showFilters&&<div style={{borderTop:'1px solid '+C.border,paddingTop:14,display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          ...
          {ANGEBOTE.map(a=><Chip key={a.v} label={a.l} active={angebotsart===a.v} onClick={()=>setAngebotsart(a.v)} col={C.violet} />)}
        </div>
        <label onClick={()=>setZeitarbeit(z=>!z)} style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',userSelect:'none'}}>
          ...
          <span style={{fontSize:13,color:C.muted}}>Zeitarbeit einschließen</span>
        </label>
      </div>}
    </Card>
    ...
    {hasSearched&&<div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12,flexWrap:'wrap'}}>
    ...
    {!hasSearched&&<div style={{textAlign:'center',padding:'80px 20px',color:C.faint}}>
      <div style={{fontSize:44,marginBottom:14,opacity:.3}}>⌕</div>
      <p style={{fontSize:15,color:C.muted}}>Suchbegriff eingeben und suchen</p>
      {vincereConnected&&<p style={{fontSize:12.5,marginTop:6}}>Grün = bereits im Vincere · Blau = noch nicht drin</p>}
    </div>}

    <div style={{display:'flex',flexDirection:'column',gap:7}}>
      {jobs.map(job=><JobCard key={job.id} job={job} vincereNames={vincereNames} onAddVincere={onAddVincere} addingId={addingId} />)}
    </div>
    {jobs.length>0&&page<totalPages&&<div style={{textAlign:'center',paddingTop:20}}>
...
function DashboardView({activities,vincereConnected,vincereCount,setView}) {
  ...
  return <div>
    ...
    {!vincereConnected&&<div style={{marginBottom:22,padding:'16px 20px',background:'rgba(245,158,11,0.08)',border:'1px solid '+C.amberBorder,borderRadius:12,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
      <div>
        <div style={{fontSize:14,fontWeight:700,color:C.amberLight,marginBottom:4}}>⚠ Vincere nicht verbunden</div>
        <div style={{fontSize:12.5,color:C.muted}}>Verbinde dein Vincere CRM um Firmenstatus in der Jobsuche zu sehen</div>
      </div>
      <button onClick={goToVincereAuth} style={{background:C.amber,color:'#000',border:'none',borderRadius:9,padding:'10px 22px',fontSize:13,fontWeight:700,cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap'}}>
        Vincere verbinden →
      </button>
    </div>}
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:22}}>
      {[{v:vincereConnected?vincereCount:'—',l:'Firmen im Vincere',c:C.violet},{v:'∞',l:'Verfügbare Stellen (BA)',c:C.green},{v:vincereConnected?'✓':'✗',l:'Vincere Status',c:vincereConnected?C.green:C.amber},{v:activities.length,l:'Aktivitäten',c:C.blue}].map((s,i)=>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:14}}>
      <Card>
        ...
        <div style={{padding:'8px 12px'}}>
          {activities.length===0&&<div style={{padding:20,textAlign:'center',color:C.faint,fontSize:13}}>Noch keine Aktivitäten</div>}
          {activities.map(a=><div key={a.id} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'10px 6px',borderBottom:'1px solid '+C.border}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:14}}>
        <Card style={{padding:18}}>
          ...
          <div style={{display:'flex',gap:8}}>
            <input value={qs} onChange={e=>setQs(e.target.value)} onKeyDown={e=>e.key==='Enter'&&qs.trim()&&setView('search')} placeholder="z.B. Mechatroniker…"
              style={{flex:1,background:'rgba(255,255,255,0.04)',border:'1px solid '+C.border2,borderRadius:8,padding:'8px 12px',fontSize:13,color:C.text,fontFamily:'inherit',outline:'none'}} />
        ...
        {vincereConnected&&<Card style={{padding:18}}>
...
function HistoryView({history,setView}) {
  return <div>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:8}}>
      {history.map(h=><Card key={h.id} style={{padding:'13px 18px',display:'flex',alignItems:'center',gap:14}}>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    fetchVincereCompanies().then(data=>{
      ...
      setVincereNames(items.map(c=>c.name||'').filter(Boolean));
    }).catch(()=>{});
  },[]);

  const handleAddVincere=async(name)=>{
    setAddingId(name);
    const ok=await addCompanyToVincere(name);
    if(ok){
      setVincereNames(prev=>[...prev,name]);
      setActivities(a=>[{id:Date.now(),type:'crm',text:name+' → Vincere',time:new Date().toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'}),col:C.violet},...a]);
    }
    setAddingId(null);
    return ok;
  };
  ...
  return <div style={{display:'flex',height:'100vh',background:C.bg,fontFamily:"'DM Sans',sans-serif",color:C.text,overflow:'hidden'}}>
    <div style={{width:220,flexShrink:0,background:C.bg2,borderRight:'1px solid '+C.border2,display:'flex',flexDirection:'column',padding:'0 10px 14px'}}>
      ...
      <nav style={{flex:1,display:'flex',flexDirection:'column',gap:2}}>
        {nav.map(item=><button key={item.id} onClick={()=>setView(item.id)} style={{display:'flex',alignItems:'center',gap:9,width:'100%',padding:'9px 12px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',background:view===item.id?C.blueDim:'transparent',border:'1px solid '+(view===item.id?C.blueBorder:'transparent'),color:view===item.id?C.blueLight:C.muted,fontSize:13,fontWeight:view===item.id?600:400,transition:'all .12s',textAlign:'left'}}>
        ...
        <div style={{marginTop:8,borderTop:'1px solid '+C.border,paddingTop:8}}>
          {vincereConnected
            ...
            :<button onClick={goToVincereAuth} style={{display:'flex',alignItems:'center',gap:9,width:'100%',padding:'9px 12px',borderRadius:9,background:'rgba(245,158,11,0.1)',border:'1px solid '+C.amberBorder,color:C.amberLight,fontSize:12.5,fontWeight:600,cursor:'pointer',fontFamily:'inherit',textAlign:'left'}}>
