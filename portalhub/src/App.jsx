...
function parseJob(j) {
  ...
  return {
    ...
    externeUrl: j.externeUrl||j.externalUrl||null,
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{display:'flex',alignItems:'center',gap:7,flexShrink:0}}>
        ...
        <VBadge company={job.company} names={names} onAdd={()=>onAdd(job.company, job.city, job.postcode, job.website, job.jobText, job.refnr, job.externeUrl)} adding={addingId===job.company} />
      </div>
    </div>
  );
}
...
function SearchView({names,onAdd,addingId,setSH,connected}) {
  ...
  const addAll=async()=>{
    ...
    for(const j of unique){await onAdd(j.company,j.city,j.postcode,j.website,j.jobText,j.refnr,j.externeUrl);setBulkDone(d=>d+1);}
    setBulkAdding(false);
  };
...
export default function App() {
  ...
  const handleAdd=async(name, city, postcode, website, jobText, refnr, externeUrl)=>{
    setAddingId(name);
    try{
      ...
      (async()=>{
        try{
          ...
          if(!contact){
            ...
            if(website) params.set('website', website);
            if(jobText) params.set('jobText', encodeURIComponent(jobText));
            const extUrl = baDetail?.externeUrl || externeUrl;
            if(extUrl) params.set('externeUrl', encodeURIComponent(extUrl));
            const r = await fetch('/api/vincere/find-contact?'+params);
            if(r.ok){
              const d = await r.json();
