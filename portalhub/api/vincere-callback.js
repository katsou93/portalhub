// Vincere OAuth2 – Step 2: Token Exchange after Login
export default async function handler(req, res) {
  ...
  if (error) {
    ...
    const redirectUri = 'https://portalhub-coral.vercel.app/api/vincere-callback';
    ...
    const tokenRes = await fetch(`https://${domain}.vincere.io/oauth2/token`, {
                                 ...
                                 if (!tokenRes.ok) {
                                   ...
                                   return res.redirect('/?crm=error&reason=' + encodeURIComponent(JSON.stringify(tokenData)));
                                     ...
                                     // Verify token works by fetching basic company data
