...
export default async function handler(req, res) {
  ...
  if (action === 'init') {
    try {
      ...
      while (start < total) {
        const r = await fetch(
          'https://' + tenant + '.vincere.io/api/v2/company/search/fl=id,name;sort=name%20asc?keyword=&start=' + start + '&rows=500',
  ...
  if (action === 'debug') {
    ...
    const r = await fetch(
      'https://' + tenant + '.vincere.io/api/v2/company/search/fl=id,name;sort=name%20asc?keyword=&start=0&rows=500',
      { headers: vincereHeaders() }
    );
