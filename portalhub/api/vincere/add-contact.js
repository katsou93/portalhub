export default async function handler(req, res) {
  ...
  if (!resolvedCompanyId && companyName) {
    try {
      ...
      // Scan pages of 10 (Vincere max) until company found or past alphabetical target
      for (let start = 0; start <= 200 && !found; start += 10) {
        const sr = await fetch(`https://${tenant}.vincere.io/api/v2/company/search/fl=id,name;sort=name asc?keyword=&start=${start}&rows=10`, { headers: h });
        if (!sr.ok) continue;
